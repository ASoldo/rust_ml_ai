  ...
  <div class="pointer-events-none absolute top-0 left-0 w-full p-3 flex items-start gap-3 z-50">
    ...
    <div
      ...
      <div class="mt-2 flex gap-2">
        <button @click="toggleFrustum()"
          class="hidden px-2 py-1 rounded bg-slate-800 hover:bg-slate-700 border border-slate-600 text-xs">Toggle
          Frustum</button>
      </div>
        ...
        <div class="mt-1 text-sm">
          ...
          <div class="max-h-48 overflow-y-auto pr-1">
            <template x-for="d in uiDetections" :key="d.key">
              <div
                class="py-1 grid grid-cols-[auto,1fr] gap-x-2 gap-y-0.5 items-center border-b border-slate-700/40 last:border-0">
